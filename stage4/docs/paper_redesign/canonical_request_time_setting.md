...
The availability-policy isolation now passes 10/10 neutral states. RT sensitivity remains blocked by missing frozen chain-bound parameters, not by the neutral identity test. The existing transformation must be applied to the same Test31 physical states; neither a guessed RT-Base label nor the old aggregate mean leads is an acceptable replacement.
