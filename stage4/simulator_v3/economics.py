...
class EconomicsModel:
    ...
    def evaluate(self, request: RequestState, vehicle: VehicleState, pickup_distance_m: float, pickup_time_sec: float, wait_sec: float, strategy: str) -> EdgeEconomics:
        ...
        if vehicle.vehicle_type == "AV":
            contribution = fare - pickup_cost - service_cost
            return EdgeEconomics(fare, 0.0, pickup_cost, service_cost, 0.0, 0.0, contribution, passenger_gc, 0.0, stress)
        ...
        platform_cost = base_payout + pickup_comp + gross_comp * 0.5 + 0.1 * route_km
        contribution = fare - platform_cost
        return EdgeEconomics(fare, driver_payout, pickup_cost, service_cost, 0.0, 0.0, contribution, passenger_gc, driver_utility, stress)
