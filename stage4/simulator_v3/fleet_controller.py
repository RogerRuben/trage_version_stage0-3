...
class FleetController:
    ...
    def build_edges(self, state: SystemState, now: pd.Timestamp, strategy: str) -> tuple[list[CandidateEdge], dict]:
        pending = [state.requests[oid] for oid in list(state.pending_request_ids) if state.requests[oid].status.value == "PENDING"]
        vehicles = self.candidates.controllable_vehicles(state.vehicles)
        ...
        for req in pending:
            for vehicle in coarse.get(req.order_id, []):
                pickup_route = self.routing.query(
                ...
                pickup_arrival = now + pd.Timedelta(seconds=pickup_route.expected_travel_time_sec)
                pickup_departure = pickup_arrival
                service_end = pickup_departure + pd.Timedelta(seconds=req.predicted_service_time_sec)
                validation = self.validator.validate(vehicle, req, now, pickup_arrival, service_end)
                if not validation.feasible:
                    validation_failures[validation.failure_reason] = validation_failures.get(validation.failure_reason, 0) + 1
                    continue
                ...
                if econ.passenger_gc > self.economics.passenger_gc_cap:
                    validation_failures["passenger_gc_cap"] = validation_failures.get("passenger_gc_cap", 0) + 1
                    continue
                if vehicle.vehicle_type == "HV":
                    ...
                    if response.response.value != "ACCEPT":
                        validation_failures[f"driver_{response.reason}"] = validation_failures.get(f"driver_{response.reason}", 0) + 1
                        continue
                if req.first_candidate_time is None:
