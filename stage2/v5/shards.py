...
def build_shards(config: Stage2V5Config, *, repo_root: str | Path = ".", output_root: str | Path = "stage2/output_v5/tensor_shards", resume: bool = False) -> dict[str, Any]:
    ...
    artifact_path = root / "stage2/output_v4/tensor_shards/feature_artifacts.json"
    artifacts = json.loads(artifact_path.read_text(encoding="utf-8"))
    if artifacts.get("fit_dates") != config.section("split")["train_dates"]:
        raise Stage2V5ContractError("frozen v4 feature artifacts do not match v5 Train dates")
    split = config.section("split")
    v5_artifacts = {
        **artifacts,
        "schema_version": "stage2_v5_tensor_artifacts.1",
        "stage2_v5_config_sha256": config.digest,
        "source_v4_artifact_sha256": _sha256(artifact_path),
    }
    _atomic_json(output / "feature_artifacts.json", v5_artifacts)
    ...
    for split_name, date in dates:
        ...
        source_split = "test" if split_name == "legacy" else split_name
        frame = load_v5_day(date, split=source_split, repo_root=root)
    ...
    overall = {
        ...
        "v4_feature_artifact_sha256": _sha256(artifact_path),
