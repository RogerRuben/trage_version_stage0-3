"""Create daily geometric-vs-HMM quality tables and Markdown reports."""
...
def plot_cases(comparison: pd.DataFrame, args: argparse.Namespace, out_dir: Path) -> None:
    ...
    for label, row in candidates:
        ...
        ax.plot(hmm.proj_x_hmm, hmm.proj_y_hmm, color="#1b9e77", linewidth=1.5, label="HMM/Viterbi")
...
def main() -> None:
    ...
    if not files:
        raise FileNotFoundError(f"no HMM quality partitions in {args.hmm_quality_dir}")
    quality_frames = []
    for path in files:
    ...
    comparison["hmm_matching_success"] = (
    ...
    out_dir = args.output_root / "matcher_comparison" / f"day={args.date}"
    ...
    summary = {
        ...
        "hmm_route_ratio_good": float(comparison.route_length_ratio.between(0.8, 1.3).mean()),
        "geometric_topology_gaps": int(comparison.geo_topology_gap_count.fillna(0).sum()),
        "hmm_topology_gaps": int(comparison.topology_gap_count.fillna(0).sum()),
        "geometric_failure_ratio": float((~comparison.geo_matching_success.fillna(False)).mean()),
        "hmm_failure_ratio": float((~comparison.hmm_matching_success).mean()),
    ...
    worker_seconds = []
    for path in manifest_dir.glob(f"day={args.date}.hmm.worker=*.json"):
    ...
    summary["hmm_runtime_seconds"] = max(worker_seconds) if worker_seconds else None
...
| Metric | Geometric/topology | HMM/Viterbi |
|---|---:|---:|
| Order P90 distance P50 | {summary['geometric_p90_distance_distribution']['p50']:.2f} m | {summary['hmm_p90_distance_distribution']['p50']:.2f} m |
| Order P90 distance P90 | {summary['geometric_p90_distance_distribution']['p90']:.2f} m | {summary['hmm_p90_distance_distribution']['p90']:.2f} m |
| Order P90 distance P95 | {summary['geometric_p90_distance_distribution']['p95']:.2f} m | {summary['hmm_p90_distance_distribution']['p95']:.2f} m |
| Route ratio in 0.8–1.3 | {summary['geometric_route_ratio_good']:.2%} | {summary['hmm_route_ratio_good']:.2%} |
| Topology gaps | {summary['geometric_topology_gaps']:,} | {summary['hmm_topology_gaps']:,} |
| Failure ratio | {summary['geometric_failure_ratio']:.2%} | {summary['hmm_failure_ratio']:.2%} |
| HMM fallback ratio | — | {summary['fallback_ratio']:.2%} |
| Runtime | {summary['geometric_runtime_seconds'] if summary['geometric_runtime_seconds'] is not None else 'not recorded'} s | {summary['hmm_runtime_seconds'] if summary['hmm_runtime_seconds'] is not None else 'not recorded'} s |
...
"""
    ...
    quality_report = args.output_root / "hmm_quality_reports" / f"day={args.date}.md"
